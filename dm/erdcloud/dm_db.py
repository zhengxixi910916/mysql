...
class DB:
    ...
    def get_connection(self, section=DEFAULT_SECTION):
        if self.connection_map.get(section) is None:
            ...
            try:
                ...
                connection = dmPython.connect(host=data.get("host"),
                                             port=int(data.get('port')),
                                             user=data.get('user'),
                                             password=data.get('password'))
                                             # db=data.get('db_name'),
                                             # charset='utf8mb4',
                                             # cursorclass=dmPython.cursors.DictCursor)
                self.connection_map[section] = connection
            except dmPython.Error.OperationalError as e:
                print("dmPython Error %d: %s" % (e.args[0], e.args[1]))
    ...
    def clear(self, section=DEFAULT_SECTION, table=None, eid=None):
        ...
        connection.commit()
    # clear table data
    def clear_bpm(self, section=DEFAULT_SECTION, table=None, eid=None):
        if table is None:
            return
        delete_sql = "delete from " + table + " where ID_ ='" + eid + "';"
        print(delete_sql)
        connection = self.get_connection(section)
        connection.cursor().execute(delete_sql)
        connection.commit()
    ...
    # 读取文件插入数据
    def readSql(self, filePath, params, section=DEFAULT_SECTION):
        if filePath is None:
            return {}
        connection = self.get_connection(section)
        cur = connection.cursor()  # 获取游标
